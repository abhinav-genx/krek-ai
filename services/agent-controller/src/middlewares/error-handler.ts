...
const SERVICE = "agent-controller";

// One line per request; 5xx are logged as errors so failures stand out.
export const requestLogger: RequestHandler = (req, res, next) => {
  const startedAt = Date.now();
  // Redact any auth token that rides in the query string (SSE stream endpoint).
  const url = req.originalUrl.replace(/(authorization=)[^&]+/i, "$1<redacted>");
  res.on("finish", () => {
    const ms = Date.now() - startedAt;
    const line = `[${SERVICE}] ${req.method} ${url} -> ${res.statusCode} (${ms}ms)`;
