    ...
    if (!chat_id) {
      ...
      const new_chat = await prisma.chat.create({
        data: {
          ...
          status: "running",
          // Remember which repos were cloned so the "Create PR" flow knows its
          // targets later.
          repos: selectedRepos,
        },
      });
...
// Keep the chat's sandbox alive while a workspace pane (editor/browser) is open.
// The sandbox lifetime is an absolute timer, so without periodic refreshes the
// sandbox is reclaimed mid-session and the panes disconnect. The frontend pings
// this on an interval; it just refreshes the timeout (no install/ready work).
chatRouter.post("/chat/keepalive", requireUserDetails, async (req, res) => {
  const userId = res.locals.user.id;
  const { chat_id } = req.body ?? {};
  ...
  if (!chat.sandboxId) {
    return res.status(409).json({ ok: false, error: "No sandbox for chat" });
  }

  try {
    const r = await axios.post(
      `${process.env.SANDBOX_CONTROLLER_URL}/sandbox/keepalive`,
      { sandboxId: chat.sandboxId },
      { headers: internalHeaders() },
    );
    return res.json({ ok: true, ...r.data });
  } catch (error) {
    // 410 from the sandbox-controller means the sandbox already expired; surface
    // it so the frontend can reopen the pane (which recreates the sandbox).
    const status =
      axios.isAxiosError(error) && error.response?.status === 410 ? 410 : 502;
    return res.status(status).json({ ok: false, expired: status === 410 });
  }
});

// A slug for the PR branch derived from the chat, e.g. "krek-ai/1a2b3c4d".
const prBranchForChat = (chatId: string): string =>
  `krek-ai/${chatId.replace(/[^a-zA-Z0-9]/g, "").slice(0, 8) || "changes"}`;

// Pre-fills the Create PR dialog: which repos the chat targets, a suggested
// branch/title/body (from the chat), and a per-repo changed-file preview.
chatRouter.post("/chat/pr-info", requireUserDetails, async (req, res) => {
  const userId = res.locals.user.id;
  const { chat_id } = req.body ?? {};
  ...
  const hasGithub = (await getGithubToken(userId)) !== undefined;
  const branch = prBranchForChat(chat_id);
  const title = chat.title?.trim()
    ? `krek-ai: ${chat.title.trim().slice(0, 60)}`
    : "krek-ai: automated changes";
  const summary = chat.result?.trim().slice(0, 1500) ?? "";
  const body = [
    summary || "Changes generated by the krek-ai agent.",
    "",
    "---",
    "_Opened by krek-ai._",
  ].join("\n");

  // Best-effort diff preview — never fail the dialog if the sandbox is slow/gone.
  let repoStatus: unknown[] = [];
  if (chat.repos.length > 0) {
    try {
      const r = await axios.post(
        `${process.env.SANDBOX_CONTROLLER_URL}/sandbox/pr-status`,
        { sandboxId: chat.sandboxId, chatId: chat_id, repos: chat.repos },
        { headers: internalHeaders(), timeout: 60_000 },
      );
      repoStatus = r.data?.repos ?? [];
    } catch {
      repoStatus = [];
    }
  }
  ...
  return res.json({
    repos: chat.repos,
    hasGithub,
    suggested: { branch, title, body },
    status: repoStatus,
  });
});

// Opens a pull request for the chat's repos: commits + pushes the agent's changes
// and creates the PR via GitHub. Only the chat's owner may call it; needs a
// connected GitHub account.
chatRouter.post("/chat/create-pr", requireUserDetails, async (req, res) => {
  const userId = res.locals.user.id;
  const { chat_id, title, body, branch, repos } = req.body ?? {};
  ...
  const chat = await prisma.chat.findUnique({
    where: { id: chat_id },
    select: { userId: true, sandboxId: true, repos: true },
  });
  ...
  // Only allow repos the chat actually cloned (defense against arbitrary targets).
  const requested: string[] = Array.isArray(repos)
    ? repos.filter((r): r is string => typeof r === "string")
    : chat.repos;
  const targetRepos = requested.filter((r) => chat.repos.includes(r));

  if (targetRepos.length === 0) {
    return res
      .status(400)
      .json({ error: "No repositories are associated with this chat" });
  }

  const githubToken = await getGithubToken(userId);
  if (!githubToken) {
    return res.status(400).json({
      error: "Connect your GitHub account to open a pull request.",
    });
  }

  try {
    const r = await axios.post(
      `${process.env.SANDBOX_CONTROLLER_URL}/sandbox/create-pr`,
      {
        sandboxId: chat.sandboxId,
        chatId: chat_id,
        repos: targetRepos,
        branch:
          typeof branch === "string" && branch.trim()
            ? branch.trim()
            : prBranchForChat(chat_id),
        title: title.trim(),
        body: typeof body === "string" ? body : "",
        githubToken,
      },
      { headers: internalHeaders(), timeout: 180_000 },
    );
    // The sandbox may have been recreated — keep the chat pointed at the live one.
    return res.json({ results: r.data?.results ?? [] });
  } catch (error) {
    const message =
      axios.isAxiosError(error) && error.response?.data?.error
        ...
        : error instanceof Error
          ? error.message
          : "Unknown error";
    console.error("POST /chat/create-pr failed:", message);
    return res.status(502).json({ error: "Failed to create pull request" });
  }
});
